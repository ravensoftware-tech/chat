...
import tech.ravensoftware.chat.crypto.axolotl.SQLiteAxolotlStore;
...
import tech.ravensoftware.chat.services.ShortcutService;
import tech.ravensoftware.chat.utils.CryptoHelper;
import tech.ravensoftware.chat.utils.CursorUtils;
import tech.ravensoftware.chat.utils.FtsUtils;
...
public class DatabaseBackend extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 34 && newVersion >= 34) {
            ...
            final File oldFilesDirectory =
                    new File(Environment.getExternalStorageDirectory() + "/Chat/");
        ...
        if (QuickChatService.isQuicksy() && oldVersion < 43 && newVersion >= 43) {
            List<Account> accounts = getAccounts(db);
            for (Account account : accounts) {
    ...
    public CopyOnWriteArrayList<Conversation> getChat(int status) {
    ...
    public ArrayList<Message> getMessages(Conversation chat, int limit) {
        return getMessages(chat, limit, -1);
    }
    ...
    public List<FilePath> getRelativeFilePaths(String account, Jid jid, int limit) {
        ...
        final String SQL =
                "select uuid,relativeFilePath from messages where type in (1,2,5) and deleted=0 and"
                        + " "
                        + Message.RELATIVE_FILE_PATH
                        + " is not null and conversationUuid=(select uuid from chat where"
    ...
    public String findConversationUuid(final Jid account, final Jid jid) {
        ...
        try (final Cursor cursor =
                db.rawQuery(
                        "SELECT chat.uuid FROM chat JOIN accounts ON"
                            + " chat.accountUuid=accounts.uuid WHERE accounts.username=?"
    ...
    public MamReference getLastMessageReceived(Account account) {
        ...
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            String sql =
                    "select messages.timeSent,messages.serverMsgId from accounts join chat"
                        + " on accounts.uuid=chat.accountUuid join messages on"
                        + " chat.uuid=messages.conversationUuid where accounts.uuid=? and"
                        + " (messages.status=0 or messages.carbon=1 or messages.serverMsgId not"
                        + " null) and (chat.mode=0 or (messages.serverMsgId not null and"
    ...
    public long getLastTimeFingerprintUsed(Account account, String fingerprint) {
        String SQL =
                "select messages.timeSent from accounts join chat on"
                        + " accounts.uuid=chat.accountUuid join messages on"
                        + " chat.uuid=messages.conversationUuid where accounts.uuid=? and"
    ...
    public List<ShortcutService.FrequentContact> getFrequentContacts(final int days) {
        ...
        final String SQL =
                "select "
                        ...
                        + " join "
                        + Message.TABLENAME
                        + " on chat.uuid=messages.conversationUuid where"
                        + " messages.status!=0 and carbon==0  and chat.mode=0 and"
                        + " messages.timeSent>=? group by chat.uuid order by count(body)"
                        + " desc limit 6;";
