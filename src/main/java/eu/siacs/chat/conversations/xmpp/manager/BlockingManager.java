...
public class BlockingManager extends AbstractManager {
    ...
    public void pushBlock(final Iq request) {
        if (connection.fromServer(request)) {
            ...
            this.removeBlockedChat(addresses);
    ...
    private void removeBlockedChat(final Collection<Jid> addresses) {
        var removed = false;
        for (final Jid address : addresses) {
            removed |= service.removeBlockedChat(getAccount(), address);
        }
        if (removed) {
    ...
    public boolean block(
        ...
        if (address.isFullJid()) {
            return false;
        } else if (service.removeBlockedChat(getAccount(), address)) {
            service.updateConversationUi();
            return true;
        } else {
            return false;
